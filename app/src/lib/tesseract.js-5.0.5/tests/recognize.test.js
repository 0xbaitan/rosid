...
describe('recognize()', () => {
  ...
  describe('should recognize with Legacy OEM', () => {
    [
      ...
      it(`recongize ${format} in base64`, async () => {
        const {
          data: { text },
        } = await workerLegacy.recognize(image);
        console.log(text);
        expect(text).to.be(ans);
      }).timeout(TIMEOUT)
    );
  ...
  describe('should recognize different langs', () => {
    [{ name: 'chinese.png', lang: 'chi_tra', ans: CHINESE_TEXT }].forEach(
      ({ name, lang, ans }) =>
    ...
    [
      {
        ...
        width: 140,
        height: 180,
        ans: SIMPLE_TEXT_HALF,
      },
    ].forEach(({ name, left, top, width, height, ans }) =>
    ...
    Object.keys(PSM)
      ...
      .forEach(({ name, mode }) =>
        it(`support PSM.${name} mode`, async () => {
          ...
          const { data } = await workerLegacy.recognize(
  ...
  (IS_BROWSER ? describe.skip : describe)(
    'should recognize image in Buffer format (Node.js only)',
    () => {
      FORMATS.forEach((format) =>
        it(`support ${format} format`, async () => {
          const buf = fs.readFileSync(
            path.join(__dirname, 'assets', 'images', `simple.${format}`)
          );
          await worker.reinitialize('eng');
          const {
            data: { text },
  ...
  (IS_BROWSER ? describe : describe.skip)(
    'should read image from img DOM element (browser only)',
    () => {
      FORMATS.forEach((format) =>
        it(`support ${format} format`, async () => {
  ...
  (IS_BROWSER ? describe : describe.skip)(
    'should read image from video DOM element (browser only)',
    () => {
      FORMATS.forEach((format) =>
        it(`support ${format} format`, async () => {
  ...
  (IS_BROWSER ? describe : describe.skip)(
    'should read video from OffscreenCanvas (browser only)',
    () => {
      // img tag is unable to render pbm, so let's skip it.
      const formats = FORMATS.filter((f) => f !== 'pbm');
